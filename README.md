...
\- \*\*HTTP Client (GUI → API):\*\* Java's built-in `java.net.http.HttpClient`
...
\- \*\*Input validation \& error handling\*\* — validation enforced at the database (constraints), service (business rules), and GUI (user-facing dialogs) layers, with custom exceptions carrying meaningful, specific error messages end-to-end
...
spring.datasource.url=jdbc:postgresql://localhost:5432/bank\_db
